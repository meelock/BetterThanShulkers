...
public class BlockElectric_fence extends Block {
	...
	public BlockElectric_fence(Material p_i46395_1_, MapColor p_i46395_2_, BtSReference.BtSBlocks blocktype) {
	...
	public boolean canConnectTo(IBlockAccess worldIn, BlockPos pos) {
		...
		return block == Blocks.BARRIER ? false
				: ((!(block instanceof BlockFence)
						|| block.getMaterial(worldIn.getBlockState(pos)) != this.blockMaterial)
						&& !(block instanceof BlockFenceGate)
								? (block.getMaterial(worldIn.getBlockState(pos)).isOpaque() && iblockstate.isFullCube()
										? block.getMaterial(worldIn.getBlockState(pos)) != Material.GOURD : false)
								: true);
	}
